...
export default function TeamPlanner({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col font-sans text-slate-900">
      ...
      {/* COMPACTE BOVENBALK */}
      <div className="p-2.5 bg-white border-b border-slate-100 flex flex-wrap items-center justify-between gap-3 select-none">
        ...
        <div className="flex items-center gap-1.5 bg-blue-50/50 border border-blue-100 px-3 py-1.5 rounded-lg text-[10px] font-semibold text-blue-800">
        ...
        <div className="flex items-center gap-2">
          <div className="relative w-56">
            ...
            <input type="text" placeholder="Zoeken op taak of persoon.." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-slate-50 border border-slate-250 rounded-lg pl-8 pr-3 py-1.5 text-[11px] focus:outline-none focus:border-blue-500 transition-colors" />
          </div>
          <button onClick={() => onAddTask('')} className="bg-blue-600 hover:bg-blue-700 text-white font-bold text-[11px] uppercase tracking-wider px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"><Plus className="w-3.5 h-3.5 stroke-[3]" /><span>Nieuw</span></button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto w-full">
        <div className="min-w-[1100px] w-full">
          ...
          {/* COMPACTE KOLOM KOPPEN */}
          <div className="grid grid-cols-12 bg-slate-50/70 border-b border-slate-100 text-[10px] font-bold text-slate-400 tracking-wider uppercase select-none">
            <div className="col-span-2 py-2 pl-4 border-r border-slate-100">Teamlid</div>
            {HOURS.map(h => (
              <div key={`head-h-${h}`} className="col-span-1 py-2 text-center border-r border-slate-100 last:border-r-0 font-mono">
            ...
            {filteredMembers.map(member => {
              ...
              return (
                <div key={member.id} className="grid grid-cols-12 items-center group hover:bg-slate-50/30 transition-colors">
                  ...
                  {/* COMPACTE LINKER KOLOM */}
                  <div className="col-span-2 py-2 pl-4 pr-2 border-r border-slate-100 flex items-center gap-2.5 select-none">
                    <div className="w-8 h-8 rounded-lg bg-blue-600 text-white font-black text-[11px] flex items-center justify-center shadow-sm shrink-0">
                      {member.initials}
                    </div>
                    <div className="truncate">
                      <h4 className="text-[11px] font-bold text-slate-800 truncate leading-tight">{member.name}</h4>
                      <p className="text-[9px] font-medium text-slate-400 font-mono leading-tight mt-0.5">
                        {activeCount} {activeCount === 1 ? 'taak' : 'taken'}
                      </p>
                    </div>
                  </div>

                  {/* COMPACTE TIMELINE (Hoogte gereduceerd) */}
                  <div className="col-span-10 h-[48px] relative bg-transparent grid grid-cols-9 divide-x divide-slate-100/70">
                    ...
                    {memberTasks.map(task => {
                      ...
                      // Aangepaste dunnere balken voor de compacte modus
                      const topOffset = isCancelled ? 'top-[26px] h-[16px]' : 'top-[6px] h-[28px] z-10 shadow-sm';
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between gap-1 w-full">
                            <span className="text-[9px] font-black uppercase tracking-wider opacity-90 truncate block leading-none mt-px">
                              {isCancelled ? '[GEANNULEERD] ' : ''}{task.subject || 'TODO'}
                            </span>
                            <span className="text-[8px] font-mono font-bold opacity-75 shrink-0 leading-none mt-px">
                              {task.startTime}-{task.endTime}
                            </span>
                          </div>
                          {task.description && task.description !== task.subject && (
                            <p className="text-[9px] font-medium opacity-90 truncate leading-none mt-1">
                              {task.description}
                            </p>
                          )}
                        </div>
          ...
          {/* COMPACTE ONDERSTE BALK */}
          <div className="grid grid-cols-12 bg-slate-50 border-t border-b border-slate-100 items-center select-none">
            <div className="col-span-2 py-2 pl-4 text-[9px] font-black text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
              <span>🔒 Totaal Schematisch</span>
            </div>
            
            <div className="col-span-10 grid grid-cols-9 h-10 divide-x divide-slate-100">
              {HOURS.map(h => {
                ...
                return (
                  <div key={`total-h-${h}`} className="h-full flex items-center justify-center bg-transparent">
                    <span className={`w-6 h-6 rounded font-mono font-bold text-[10px] flex items-center justify-center ring-1 ring-inset ${badgeColor}`}>
                      {activeTasksInHour}
                    </span>
                  </div>
                );
          ...
          <div className="py-2.5 px-4 bg-slate-50/50 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3 text-[9px] font-bold text-slate-400 tracking-wide select-none">
            <div className="flex items-center gap-3">
              <span className="uppercase tracking-wider">Kleurwisser:</span>
              <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-rose-600" /><span>Urgent / Verlof</span></div>
              <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-orange-500" /><span>Hoog</span></div>
              <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-amber-500" /><span>Medium</span></div>
              <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-purple-600" /><span>Ziekte</span></div>
              <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-emerald-500" /><span>Laag</span></div>
              <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-slate-300" /><span>Geannuleerd</span></div>
            </div>
